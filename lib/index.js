...
const unmatchedDisposal = (report, disposal, localCurrency) => {
  ...
  reportToUpdate = report.updateIn([disposalYear, 'unmatched'], list => list.push(sale.set('transaction_id', disposal.transactionId)));
...
const setupAssetProperties = (year, report, yearDisposals, allLots) => {
  const unixSOY = Number(external_moment_default.a.utc(year, 'YYYY').startOf('year').format('X'));
  const unixEOY = Number(external_moment_default.a.utc(year, 'YYYY').endOf('year').format('X'));
  const reportWithBought = allLots.reduce((accReport, assetLots, assetKey) => {
    ...
    const existingSold = accReport.getIn([year, 'assets', assetKey, 'bought'], new external_bignumber_js_["BigNumber"](0));
    const existingHoldings = accReport.getIn([year, 'assets', assetKey, 'holdings'], new external_bignumber_js_["BigNumber"](0));
    return accReport.setIn([year, 'assets', assetKey, 'bought'], assetReduction).setIn([year, 'assets', assetKey, 'sold'], existingSold).setIn([year, 'assets', assetKey, 'holdings'], existingHoldings);
  }, report);
  const reportWithSold = yearDisposals.reduce((accReport, disposal) => {
    ...
    const existingSold = accReport.getIn(keypathSold, new external_bignumber_js_["BigNumber"](0));
    const existingBought = accReport.getIn(keypathBought, new external_bignumber_js_["BigNumber"](0));
    const existingHoldings = accReport.getIn(keypathHoldings, new external_bignumber_js_["BigNumber"](0));
    return accReport.setIn(keypathSold, external_bignumber_js_["BigNumber"].sum(existingSold, disposal.assetAmount)).setIn(keypathBought, existingBought).setIn(keypathHoldings, existingHoldings);
  }, reportWithBought);
...
function bigNumberToString(obj, base, places = 2) {
  ...
  if (typeof obj === 'object' && obj !== null && !Array.isArray(obj) && !('lessThan' in obj)) {
    Object.keys(obj).forEach(function (key) {
      obj[key] = bigNumberToString(obj[key], base, ['bought', 'sold', 'holdings', 'asset_amount'].includes(key) ? 18 : undefined);
    });
  }
...
const lotsAndDisposalsFromTrade = ({
  ...
  let feeCode;
  let feeAmount = new external_bignumber_js_["BigNumber"]('0');
  let feePrice;
  let taxableFeeAmount;

  if (hasFee) {
    feeCode = transactionFeeCode(transaction);
    feeAmount = new external_bignumber_js_["BigNumber"](transaction.get('fee_amount'));
  ...
  let lotAmount = new external_bignumber_js_["BigNumber"](transaction.get(`${lotSide}_amount`));
  ...
  let disposalAmount = new external_bignumber_js_["BigNumber"](transaction.get(`${disposalSide}_amount`));

  if (hasFee && feeCode === disposalCode) {
    disposalAmount = external_bignumber_js_["BigNumber"].sum(disposalAmount, feeAmount);
  }

  const disposalList = Object(external_immutable_["List"])([new src_disposal({
    unix: unixNumber,
  ...
  let otherDisposals = Object(external_immutable_["List"])();

  if (hasFee && feeCode !== disposalCode && feeCode !== lotCode) {
    otherDisposals = otherDisposals.push(new src_disposal({
      unix: unixNumber,
      assetCode: feeCode,
...
const lotsAndDisposalsFromDeposit = ({
  ...
  let disposalList = Object(external_immutable_["List"])();

  if (hasFee && feeCode !== lotCode) {
    disposalList = Object(external_immutable_["List"])([new src_disposal({
      unix: unixNumber,
      assetCode: feeCode,
      assetAmount: feeAmount,
      proceedsCode: localCurrency,
...
const lotsAndDisposalsFromWithdrawal = ({
  ...
  if (hasFee && feeCode === withdrawalCode) {
    withdrawalAmount = external_bignumber_js_["BigNumber"].sum(withdrawalAmount, feeAmount);
  }
...
const lotsAndDisposalsFromLost = ({
  ...
  const withdrawal = Object(external_immutable_["Map"])({
...
const lotsAndDisposalsFromMint = ({
  ...
  const trade = Object(external_immutable_["Map"])({
...
const lotsAndDisposalsFromCompoundRedeem = ({
  ...
  return lotsAndDisposalsFromTrade({
    transaction: Object(external_immutable_["Map"])({
...
const createReport = options => {
  ...
  const transactionsList = Object(external_immutable_["fromJS"])(transactions);
  const pricesList = Object(external_immutable_["fromJS"])(prices);
...
const buildReport = options => {
  ...
  const output = reportIterable.reduce((acc, iterableYear) => {
    ...
    const reportWithBoughtSold = setupAssetProperties(iterableYear.get('year'), reportIncomeSetup, iterableYear.get('disposals'), acc.get('lots'));
...
const buildReportYear = options => {
  ...
  return sortedDisposals.reduce((acc, disposal) => {
    ...
    const beforeDisposal = lot => lot.unix <= disposal.unix;
